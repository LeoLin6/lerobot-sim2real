...
@register_env("LegoMinifigurePickPlace-v1", max_episode_steps=100)
    ...
    **Success Conditions:**
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            minifig_xyz = torch.zeros((b, 3))
            minifig_xyz[:, :2] = (
                torch.rand((b, 2)) * self.spawn_box_half_size * 2
                - self.spawn_box_half_size
            )
            minifig_xyz[:, :2] += spawn_box_pos[env_idx, :2]
            minifig_xyz[:, 2] = 0  # Height for real-sized minifigure

            # Minifigure orientation
            random_z_angle = torch.rand(b) * 2 * np.pi
            random_z_angles = random_z_angle.cpu().numpy()
            qs = [euler2quat(np.pi/2, 0, angle) for angle in random_z_angles]
            qs = np.array(qs)
            self.minifigure.set_pose(Pose.create_from_pq(minifig_xyz, qs))

            # Initialize fighter in same region, but not intersecting minifigure
            fighter_xyz = torch.zeros((b, 3))
            min_distance = 0.1  # Minimum allowed distance between minifigure and fighter

            for i in range(b):
                for attempt in range(50):
                    candidate = (
                        torch.rand(2) * self.spawn_box_half_size * 2
                        - self.spawn_box_half_size
                    )
                    candidate += spawn_box_pos[env_idx[i], :2]
                    if torch.linalg.norm(candidate - minifig_xyz[i, :2]) >= min_distance:
                        fighter_xyz[i, :2] = candidate
                        break
                else:
                    # fallback: just use a random position if all attempts fail
                    fighter_xyz[i, :2] = (
                        torch.rand(2) * self.spawn_box_half_size * 2
                        - self.spawn_box_half_size
                    ) + spawn_box_pos[env_idx[i], :2]
            fighter_xyz[:, 2] = 0  # Height for fighter
            ...
            qs = [euler2quat(np.pi/2, 0, angle) for angle in random_z_angles]
            qs = np.array(qs)
            self.fighter.set_pose(Pose.create_from_pq(fighter_xyz, qs))
    ...
    def evaluate(self):
        ...
        fighter_above = self.fighter.pose.p.clone()
        fighter_above[..., 2] += 0.05  # Add 0.05 to z-coordinate

        tcp_to_fighter_above_dist = torch.linalg.norm(
            fighter_above - self.agent.tcp_pose.p, axis=1
        )

        reached_minifigure = tcp_to_minifigure_dist < 0.03
        reached_fighter = tcp_to_fighter_above_dist < 0.03

        is_grasped = self.agent.is_grasping(self.minifigure)

        # Minifigure is lifted if it's above the table
        minifigure_lifted = self.minifigure.pose.p[..., -1] >= 0.04

      # Success: minifigure is inside the fighter
        success = is_grasped &  minifigure_lifted & reached_fighter 
        ...
        # Check if robot is touching fighter
        l_contact_forces_fighter = self.scene.get_pairwise_contact_forces(
            self.agent.finger1_link, self.fighter
        )
        r_contact_forces_fighter = self.scene.get_pairwise_contact_forces(
            self.agent.finger2_link, self.fighter
        )
        lforce_fighter = torch.linalg.norm(l_contact_forces_fighter, axis=1)
        rforce_fighter = torch.linalg.norm(r_contact_forces_fighter, axis=1)
        touching_fighter = torch.logical_or(
            lforce_fighter >= 1e-2,
            rforce_fighter >= 1e-2,
        )
        ...
        return {
            ...
            "tcp_to_fighter_above_dist": tcp_to_fighter_above_dist,
            "touching_table": touching_table,
            "minifigure_lifted": minifigure_lifted,
            #"touching_fighter": touching_fighter,
            "success": success,
        }
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        # Distance to spot above fighter (e.g., 0.05 units above)
        fighter_above = self.fighter.pose.p.clone()
        fighter_above[..., 2] += 0.05  # Add 0.05 to z-coordinate
        ...
        place_reward = torch.exp(-2 * tcp_to_fighter_above_dist)
        reward += place_reward * info["is_grasped"]
        reward -= 2 * info["touching_table"].float()
        #reward -= 2 * info["touching_fighter"].float()
        return reward
